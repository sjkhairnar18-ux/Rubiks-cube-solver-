...
public:
     ...
     * getter function not setter hence marked const
     */
     ...
     * static function as doesn't need to be called on an object
     */
     ...
     * Return the moves for: 
     * 1. Study which scramble types are harder to solve
     * 2. Analyze correlation between scramble length and solution complexity
     * 3. Generate consistent test cases for algorithm comparison
     */
    ...
    //Unsigned 8 bit integer is used to represent the index of the corner. Memory Efficient: 1 byte vs 4 bytes for int(0 - 255)
    //0-7 are the indices of the corners
    //0-3 are the indices of the corners on the top face (U)
    //4-7 are the indices of the corners on the bottom face (D)
    //0-3 are the indices of the corners on the left face (L)
    //4-7 are the indices of the corners on the right face (R)
    //0-3 are the indices of the corners on the front face (F)
    //4-7 are the indices of the corners on the back face (B)
