...
char RubiksCube::getColorLetter(COLOR color) { //:: scope resolution. This is a func that is of the RubiksCube class.
    switch (color) {
        case COLOR::BLUE:
            return 'B';
        case COLOR::GREEN:
...
RubiksCube& RubiksCube::move(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return this->l(); //l() is a function that rotates the left face of the cube and this is a pointer to the current object
        case MOVE::LPRIME:
            return this->lPrime(); 
        case MOVE::L2:
            return this->l2();
...
void RubiksCube::print() const {
    ...
    //       W W W     ← 7 spaces + UP face row 0
    //       W W W     ← 7 spaces + UP face row 1  
    //       W W W     ← 7 spaces + UP face row 2
    ...
    // G G G  R R R  B B B  O O O     ← Row 0 of all 4 faces
    // G G G  R R R  B B B  O O O     ← Row 1 of all 4 faces  
    // G G G  R R R  B B B  O O O     ← Row 2 of all 4 faces
...
vector<RubiksCube::MOVE> RubiksCube::randomShuffleCube(unsigned int times) {
    ...
    for (unsigned int i = 0; i < times; i++) {
        ...
        moves_performed.push_back(static_cast<MOVE>(selectMove)); //static_cast<MOVE>(selectMove) is a function that converts the selectMove(int) to a MOVE(enum) type
...
// Takes a corner index (0-7) and returns a 3-character string with the colors of that corner piece
// 0: UFR, 1: UFL, 2: UBL, 3: UBR, 4: DFR, 5: DFL, 6: DBR, 7: DBL
...
// The cube has 3 pairs of opposite colors:
// White (W) vs Yellow (Y) - opposite faces
// Red (R) vs Orange (O) - opposite faces
// Blue (B) vs Green (G) - opposite faces
// Each corner has exactly one color from each pair, creating a unique 3-bit signature:
...
// White and Yellow stickers belong on UP and DOWN faces only!
// In a correctly oriented corner:
// Position 0 = The face that should touch UP or DOWN
// Position 1 = The face that should touch a side (FRONT/BACK/LEFT/RIGHT)
// Position 2 = The face that should touch a side (FRONT/BACK/LEFT/RIGHT)
// Orientation Values:
// 0 = Correct orientation (W/Y in position 0)
// 1 = Twisted clockwise (W/Y in position 1)
// 2 = Twisted counter-clockwise (W/Y in position 2)
